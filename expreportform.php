...
PRINT EXP REPORT SHEET<br />
<h5>FILTER EXP STATEMENT OF ACTIVITIES BY CATEGORIES </h5>
</div>
<div class="panel-body">

<form action="expstatement.php" method="post" enctype="multipart/form-data" name="Activities Generator" target="_blank">
<table align="center" cellspacing="4" class="table table-bordered">
            ...
            <tr>
              <td colspan="4" class="labels"><h4>CATEGORIES 
                </h4>
                <hr /> 
                <?php $getsubcat = mysql_query("SELECT DISTINCT category FROM expenditure") or die(mysql_error());
		while($subcat = mysql_fetch_array($getsubcat)){?>
                <input type="checkbox" name="subcategory[]" value="<?php echo $subcat['category']; ?>"> <?php echo $subcat['category']; ?></br>
        <?php } ?></td>
             </tr>
            <tr>
